...
\begin{singlespacing}
    ...
    \begin{small}
            ...
            \emph{tweet\_dist}              & $Tuple$                       & \specialcell{A tuple representing a random variable \\ $X \sim\mathcal{N}(\mu,\,\sigma^{2})$ that is sampled $n$ times to determine \\ the number of tweet vertices per party leader} \\ \hline
...
\end{singlespacing}
 ...
 user and retweets $D_{i}$ tweets. For each retweet a user makes, edge
 probabilities for each type of tweet are calculated based on the user's prior
 retweet history. The probability of forming an edge between user $i$ and a
 tweet of type $t$, given edge probabilities $e_{i}$, is given by the
 ...
 $\epsilon$-$greedy$ algorithm, where the tweet-type with the highest probability
...
blockmodels. The full algorithm is given in algorithm \ref{algorithm:SBM}.
...
\begin{algorithm}[H]
    ...
    initialize $T$ as an array of all different types of tweets\;
    ...
    \For{$i\leftarrow 1$ \KwTo $m$}{
        ...
        \For{$j\leftarrow 1$ \KwTo $D_{i}$}{
            ...
            add an edge from user $i$ to a tweet of type $t$ based on $\pi(t|e_{i})$\;
            $user\_history_{t}+=1$
        } 
     }
     \KwRet{$G$}
...
\subsection{Deep Stochastic Blockmodels for Modelling Political Engagement}\label{sec:DeepSBMs}
...
%     \centering
%     \includegraphics[scale=0.3]{Figures/standard_heat_trace_plot}
%     \caption[Standard Stochastic Hybrid Blockmodel Plots]{Standard Stochastic Hybrid Blockmodel Plots}
%     \label{fig:standard_heat_trace_plot}
%     \end{figure}
% \end{singlespacing}
...
%     \centering
%     \includegraphics[scale=0.3]{Figures/ann_heat_trace_plot}
%     \caption[Deep Stochastic Hybrid Blockmodel Plots]{Deep Stochastic Hybrid Blockmodel Plots}
%     \label{fig:ann_heat_trace_plot}
%     \end{figure}
% \end{singlespacing}
